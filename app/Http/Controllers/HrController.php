...
class HrController extends Controller
{
    public function index(Request $request)
    {
        ...
        // เพิ่มข้อมูล engagement และ satisfaction scores
        $employees = $employees->map(function($employee) {
            $latestSurvey = $employee->surveys->sortByDesc('ReviewID')->first();
            $employee->engagement_score = $latestSurvey ? $latestSurvey->EngagementScore : 0;
            $employee->satisfaction_score = $latestSurvey ? $latestSurvey->SatisfactionScore : 0;
            return $employee;
        });
